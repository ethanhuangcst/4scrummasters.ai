...
| 元素 | 作用 | 最佳实践 |
| --- | --- | --- |
| **S - Situation 情境（Context）** | 提供待解决问题的背景和情境, 说明任务的上下文,帮助大模型缩小假设的范围 | • 无指令：不要写任务指令，只描述背景<br>• 使用 {}：用 {粘贴完整的背景和概念} 作为占位符，让用户动态填写，提高模板可复用性 |
| **T - Task 任务（Objective）** | 提供清晰、明确、无歧义的任务指令, 让大模型清楚的理解要做的事 | • 指令清晰：一句话一个指令<br>• 动词导向：使用动词引导的指令<br>• 关键要求：添加关键要求描述（例如 SMART 原则） |
| **A - Action Role 行动角色（Role Assignment）** | 说明希望大模型扮演的角色, 和具备的专业技能 | • 专业角色：指定具备任务相关技能的角色<br>• 领域：强调角色的专业领域 |
| **R - Rule 规则（Task Rules）** |• 提供清晰、实例化、一致的任务规则<br>• 指定输出结果的格式 | • 实例化：​给​出​实例​帮助​ AI​ 理​解  AI 理解<br>• 边界清晰：将“请尽可能清楚描述”改为“必须提供至少三个场景”<br>• 质量要求：如有质量标准，明确说明<br>• 模板：指定句式/结构模板<br> • 大模型的输出格式,例如纯文本、文本块等|
...
# S - Situation
## 我是Ethan Huang
## Ethan 是 Scrum Alliance 的认证培训师（CST），也是设计思维、精益、业务创新领域的专家
## Ethan 是自由职业(Freelancer)，不擅长经营个人IP，也没有市场的背景，和数字化营销的技能
## Ethan 非常忙，没有足够的时间拍摄视频，而且镜头感很差，刻意拍摄的视频质量欠佳
## Ethan 正在使用 HeyGen 平台，快速生成自己的数字人短视频，用于微信视频号推广个人IP
## Ethan 视频期望触达的人群是比较垂直的专业领域：
- 30-45岁之间
...
---
# R - Rule
## 请在后续的对话中，等待我输入的{视频主题和内容说明}
## 针对我输入的视频主题和内容说明，设计3个自然语速 (150WPM)下55-65秒的数字人视频文字脚本
## 以 plain text 的格式，用 Markdown 语法输出到一段文字块中，方便我复制与编辑
