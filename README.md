...
| **P - Purpose 目的（Objective）** | 提供清晰、明确、无歧义的任务指令, 让大模型清楚的理解要做的事 | • 指令清晰：一句话一个指令<br>• 动词导向：使用动词引导的指令<br>• 关键要求：添加关键要求描述（例如 SMART 原则） |
| **A - Action Role 行动角色（Role Assignment）** | 说明希望大模型扮演的角色, 和具备的专业技能 | • 专业角色：指定具备任务相关技能的角色<br>• 领域：强调角色的专业领域 |
| **R - Rule 规则（Task Rules）** | 提供清晰、实例化、一致的任务规则 | • 实例化：​给​出​实例​帮助​ AI​ 理​解  AI 理解<br>• 边界清晰：将“请尽可能清楚描述”改为“必须提供至少三个场景”<br>• 质量要求：如有质量标准，明确说明 |
| **K - Knowledge Format 知识格式（Output Format）** | 定义输出格式 | • 模板：指定句式/结构模板<br> • 大模型的输出格式,例如纯文本、文本块等 |
